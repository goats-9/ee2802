...
\begin{align}
    ...
    \implies y &= \vec{\theta}^\top\vec{x} \label{eq:model}
\end{align}
where
\begin{align}
    y = V(T),\ \vec{\theta} = V(0)\myvec{1\\A\\B},\ \vec{x} = \myvec{1\\T\\T^2}
...
\begin{align}
    \min_{\vec{\theta}}\sum_{i}\abs{\vec{\theta}^\top\vec{x_i} - y_i}^2
    \label{eq:opt-model}
\end{align}
...
\begin{align}
    \vec{\theta} = \myvec{1.6547\\3.199\times10^{-3}\\-3.9599\times10^{-6}}
    \label{eq:opt-theta}
\end{align}
