...
\begin{enumerate}
    ...
    \begin{align}
        ...
        \vec{m_1}^\top\brak{\vec{A}-\vec{B}} = 0 \label{eq:L1-perp} \\
        \vec{m_2}^\top\brak{\vec{A}-\vec{B}} = 0 \label{eq:L2-perp}
    \end{align}
    \eqref{eq:L1-perp} and \eqref{eq:L2-perp} give
    \begin{align}
        \vec{M}^\top\brak{\vec{A}-\vec{B}} = \vec{O}
        \label{eq:M-a-b-eqn}
    \end{align}
    where $\vec{O} \triangleq \myvec{0\\0}$. Substituting \eqref{eq:A-def} and 
    \eqref{eq:B-def} into \eqref{eq:M-a-b-eqn}, and using \eqref{eq:M-def} and 
    \eqref{eq:lambda-def},
    \begin{align}
        \vec{M}^\top\vec{M}\vec{\lambda} = \vec{M}^\top\vec{x}
        \label{eq:M-lambda-x-eqn}
    \end{align}
    ...
    \begin{enumerate}
        ...
        \begin{align}
            ...
            \label{eq:intersect}
        \end{align}
        Thus, both lines intersect at a point in this case, and the shortest
        distance between them is trivially 0. To check whether such a $\vec{\lambda}$
        exists, we can bring the augmented matrix $\myvec{\vec{M}&\vec{x}}$ into
        row-reduced echelon form and check whether there is a pivot in the last
        column.
        ...
        equation from \eqref{eq:M-lambda-x-eqn}.
        \begin{align}
        ...
        are skew. From \eqref{eq:M-lambda-x-eqn},
        \begin{align}
        ...
        \begin{align}
            ...
            \label{eq:dist-skew}
        \end{align}
        where $\vec{I_n}$ denotes the identity matrix of order $n$.
    \end{enumerate}
\end{enumerate}
\end{document}
